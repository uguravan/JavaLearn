...
public class C11_stream_iterator {
    ...
    public static void task06(int a, int x){
       IntStream.iterate(a, t-> t*a).limit(x).forEach(Lambda_Methods::bosluklaYazdir);
    }

        //TASK 07 --> Istenilen bir sayinin faktoriyelini hesaplayan programi  create ediniz.
    public static void task07(int x){
       IntStream.rangeClosed(1,x).reduce(1, Math::multiplyExact);
    }

        //TASK 08 --> Istenilen bir sayinin  x. kuvvetini ekrana yazdiran programi  create ediniz.
    public static void task08(int a, int x){
       IntStream.iterate(a, t-> t*a).limit(x).skip(x-1).forEach(Lambda_Methods::bosluklaYazdir);
    }
